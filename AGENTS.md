...
- Posts live at root-level `YYYY/MM/DD/<kebab-slug>.md`; keep post assets beside them.
...
- The root website index and generated README use shared source content. Never parse README as site data.
...
- Public post routes are `/<YYYY>/<MM>/<DD>/<slug>` with no `.md` or `.html`.
- Astro is static-only for GitHub Pages. Preserve co-located relative images and trusted raw HTML/YouTube embeds.
- UI stays minimal and editorial: `schwarzkopfb/blog`, post-page `All posts`, a muted uppercase path label, large serif H1, simple date/title index, and chronological previous/next post links.
